    ...
    <section id="events" className="scroll-mt-20 bg-white px-5 py-14 sm:px-8 lg:px-12 lg:py-20">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-6 grid grid-cols-1 gap-3.5 sm:grid-cols-2 lg:grid-cols-3">
          {data.map((event) => (
            <article key={event.id} className="flex gap-3.5 rounded-xl border border-slate-200/80 bg-white p-4 shadow-sm">
              <div className="w-[50px] flex-none rounded-[9px] bg-blue-50 py-2 text-center">
                <div className="text-[10px] font-semibold uppercase tracking-[0.1em] text-blue-600">
                  {event.month}
                </div>
                <div className="text-xl font-extrabold leading-none text-blue-700" style={SERIF}>
                  {event.day}
                </div>
              </div>
              <div>
                <p className="text-[13.5px] font-semibold text-slate-800">{event.title}</p>
                <p className="mt-0.5 text-[11.5px] leading-[1.5] text-slate-500">{event.location}</p>
