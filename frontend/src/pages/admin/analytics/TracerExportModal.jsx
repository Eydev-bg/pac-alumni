// ═══════════════════════════════════════════════════════════
//  FILE: frontend/src/pages/admin/analytics/TracerExportModal.jsx
//  Export modal with filters (course, batch, status) and
//  format selector (Excel/CSV/PDF). Triggers blob download.
// ═══════════════════════════════════════════════════════════
...
export default function TracerExportModal({
  isOpen,
  onClose,
  departments,
  courses,
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (empStatus) params.employment_status = empStatus;

      const res = await adminApi.exportTracer(params);
      ...
      let filename = `graduate-tracer-${new Date().toISOString().slice(0, 10)}.${format}`;
      if (disposition) {
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            >
              ...
              <option value="not_registered" className={optionClass}>
                Not Registered
              </option>
            </select>
          </div>
